...
function VinceRaidFrames:ICCommSetup()
	...
	if self.channel:IsReady() then
		self.icommtimer:Stop()
		--Print("in ICCommSetup ready")
		...
		--Print("in iccommsetup else")
		self.icommtimer:Start()
	end
	--Print("end of ICCommSetup")
end
...
function VinceRaidFrames:ShareGroupLayout()
	if not self.channel then 
		--Print("VRF: Error sending message.")
		self:ICCommSetup()
		return
	end
	--Print("in Share")
	...
	--Print("Msg sent")
	self.channel:SendMessage(self.Utilities.Serialize(self:GetGrpCode()))
end
...
function VinceRaidFrames:GetGrpCode()
	if not self.settings.groups then return nil end
